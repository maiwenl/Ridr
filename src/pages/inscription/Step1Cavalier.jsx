...
export default function Step1Cavalier({data, onChange, errors}) {
    ...
    return (
        ...
        />
            ...
            <FieldOptional label="Numéro de licence FFE">
                <input
                    type="text"
                    value={data.licence_ffe ?? ''}
                    onChange={e => onChange('licence_ffe', e.target.value)}
...
function FieldOptional({label, children}) {
    return (
        <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
                {label} <span className="text-brand-400 font-normal">(facultatif)</span>
            </label>
            {children}
        </div>
    )
