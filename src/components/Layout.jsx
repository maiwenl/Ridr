...
const ICONS = {
    ...
    moncentre: "M3.75 21h16.5M4.5 3h15M5.25 3v18m13.5-18v18M9 6.75h1.5m-1.5 3h1.5m-1.5 3h1.5m3-6H15m-1.5 3H15m-1.5 3H15M9 21v-3.375c0-.621.504-1.125 1.125-1.125h3.75c.621 0 1.125.504 1.125 1.125V21",
...
const NAV_BY_ROLE = {
    gérant: [
        {to: '/mon-centre', label: 'Mon club', iconKey: 'moncentre'},
        {to: '/adhesions', label: 'Adhérents', iconKey: 'adhesions'},
        {to: '/cours', label: 'Activités', iconKey: 'cours'},
...
function CollapsedTooltip({children}) {
    return (
        <span
            className="pointer-events-none absolute left-full ml-3 z-50 opacity-0 group-hover:opacity-100 transition-opacity duration-150 whitespace-nowrap bg-slate-800 text-white text-xs font-medium px-2.5 py-1.5 rounded-lg shadow-lg border border-white/10">
      {children}
    </span>
    )
}
...
function UserAvatar({prenom, nom}) {
    ...
    return (
        <div
            className="w-8 h-8 rounded-full bg-brand-500/30 border border-brand-400/30 flex items-center justify-center shrink-0">
...
export default function Layout() {
    const {profile, clubSlug, signOut} = useAuth()
    ...
    return (
        <div className="h-screen flex flex-col md:flex-row bg-gray-50 overflow-hidden">
            ...
            {/* ── Sidebar desktop ─────────────────────────────────────────────────── */}
            <aside
                className={`hidden md:flex flex-col shrink-0 bg-slate-900 shadow-sidebar transition-all duration-300 ease-in-out h-screen ${collapsed ? 'w-[68px]' : 'w-60'}`}>

                {/* Logo + toggle */}
                <div
                    className={`flex items-center h-[64px] shrink-0 ${collapsed ? 'justify-center px-0' : 'justify-between px-4'}`}>
                    {!collapsed && <Logo/>}
                    <button
                ...
                {collapsed && saisons.length > 0 && (
                    <div className="px-2.5 pb-2.5">
                        <div className="group relative flex items-center justify-center">
                            <div
                                className="w-10 h-10 rounded-lg flex items-center justify-center bg-slate-800 border border-slate-700/60">
                <span className="text-[10px] font-bold text-slate-300 leading-none text-center">
                  {saisonCourante?.name?.slice(-4) ?? '—'}
                </span>
                            </div>
                            <CollapsedTooltip>{saisonCourante?.name ?? 'Aucune saison'}</CollapsedTooltip>
                        </div>
                    </div>
                )}
                ...
                {/* Nav */}
                <nav
                    className={`flex-1 py-2 overflow-y-auto overflow-x-hidden space-y-0.5 ${collapsed ? 'px-2' : 'px-3'}`}>
                ...
                <div className={`py-3 border-t border-white/[0.06] space-y-0.5 ${collapsed ? 'px-2' : 'px-3'}`}>
                    {profile && (
                        <div
                            className={`flex items-center mb-1 ${collapsed ? 'justify-center py-2' : 'gap-3 px-3 py-2.5'}`}>
                            {collapsed ? (
                                <div className="group relative">
                                    <UserAvatar prenom={profile.first_name} nom={profile.last_name}/>
                                    <CollapsedTooltip>{profile.first_name} {profile.last_name} · <span
                                        className="capitalize">{profile.role}</span></CollapsedTooltip>
                                </div>
                            ) : (
                                <>
                                    <UserAvatar prenom={profile.first_name} nom={profile.last_name}/>
                    ...
                    {role === 'gérant' && clubSlug && (
                        <a
                            href={`${import.meta.env.BASE_URL}inscription/${clubSlug}`}
                            target="_blank"
                        ...
                        >
                            <Icon d={ICONS.link}
                                  className="w-[18px] h-[18px] shrink-0 text-slate-400 group-hover:text-slate-200"/>
            ...
            <div className="flex-1 flex flex-col min-w-0">
                ...
                {/* Top bar mobile */}
                <header
                    className="md:hidden bg-slate-900 px-4 py-3 flex items-center justify-between gap-3 sticky top-0 z-30">
                    <Logo/>
                    <SaisonSelect saisons={saisons} saisonCourante={saisonCourante} choisirSaison={choisirSaison}
                                  mobile/>
                </header>
            ...
            {/* ── Bottom nav mobile ────────────────────────────────────────────────── */}
            <nav
                className="md:hidden fixed bottom-0 inset-x-0 z-50 bg-slate-900/95 backdrop-blur-md border-t border-white/10">
